...
> **Project status: 🟢 Milestone 10 — Observability & Retention Enforcement (complete & verified).** On top of the M9 durable audit trail, M10 turns the M4–M9 metrics/logging/health foundations into a coherent operational layer: `/actuator/prometheus` is now exposed (`micrometer-registry-prometheus`, single new dependency; production restricts at the network layer), Kubernetes-style `/actuator/health/{liveness,readiness}` probes are enabled (readiness is DB-only — Redis and Ollama are deliberately excluded so degraded memory or an offline LLM does not mark the whole service unready), every response carries `X-Request-Id` (`RequestIdFilter` accepts only a valid UUID; otherwise mints a UUIDv4), and every log line inside an agent run additionally carries `executionId` via MDC. The full **metric taxonomy is frozen** (no M4–M9 metric renamed) and locked to a bounded set of low-cardinality tag keys — enforced by a `MetricCardinalityTest` that walks the whole registry after boot and rejects `userId`/`conversationId`/`executionId`/`requestId`/`confirmationId`/`arguments`/`prompt`. **Audit retention is now enforced:** `AuditRetentionJob` deletes `agent_executions` older than `AGENT_AUDIT_RETENTION_DAYS` (default 90) nightly at 03:15 UTC, in batches of 500 with a 100-batch per-run ceiling, children cascade via existing FKs, best-effort on failure (never crashes the app), single-node overlap-safe via `ReentrantLock.tryLock()`. Verified end-to-end with real Postgres (Testcontainers): expired executions and their steps/tool_executions are purged; fresh rows survive; a second invocation is a no-op. Dashboards, JSON logs, and multi-node purge coordination stay explicitly deferred to M12+.

> _Earlier (M9) — Durable Agent Audit & Execution History (complete & verified):_ On top of the M8 guardrails, the app now has a **durable, owner-scoped audit trail**: every agent run is recorded in PostgreSQL across three typed tables (`agent_executions`, `agent_steps`, `tool_executions`) written from backend-observed facts — execution lifecycle, LLM-decision metadata, guardrail outcomes, confirmations, and tool executions — never by the LLM and never reconstructed from prompts. A repository-free `AgentExecutionListener` seam (no-op default) keeps the agent core JPA-free; writes are best-effort in their own short transactions and never block a run. Read it back via `GET /api/v1/agent/executions[/{executionId}]` (owner-scoped, paginated, sanitized). It stores **no** raw prompts, tool arguments, LLM output, chain-of-thought, or secrets — only metadata, hashes, and bounded summaries. Verified end-to-end with real Postgres+Redis (agent → guardrail → tool → audit; confirm promotes the run; cross-user reads masked 404). Observability dashboards and retention purge are **M10**.
>
> _Earlier (M8) — Guardrails & Agent Safety Enforcement (complete & verified):_ On top of the M7 memory layer, the app has a **backend-authoritative guardrail layer**: a `GuardrailEngine` evaluates every proposed tool action before any effect (ALLOW / DENY / REQUIRE_CONFIRMATION), with tool **risk read from the descriptor** so the model can never downgrade it. SIDE_EFFECTING/HIGH_RISK actions halt at `PENDING_CONFIRMATION` and run only after an explicit, **single-use, owner-scoped, fingerprint-bound** confirmation (`POST /api/v1/agent/confirmations/{id}`, executed **exactly once**, no automatic loop resume). A per-user fixed-window rate limit caps tool calls; timeouts are layered and never force-cancel an in-flight write. Confirmation and rate state live in their own Redis namespaces (`guard:confirmation:{id}`, `guard:rate:{userId}:{epochMinute}`). Verified end-to-end with real Postgres+Redis (execute → confirm → task created exactly once; replay creates none; cross-user confirm masked 404). M8 does **not** claim to solve prompt injection — the boundary is structural. Durable audit is **M9**; observability dashboards are **M10**.
>
> _Earlier (M7) — Redis Conversation Memory (complete & verified):_ On top of the M6 agent, the app has **short-term conversation memory**: `POST /api/v1/agent/execute` accepts an optional server-minted UUID `conversationId` (absent → new conversation) and returns `conversationId` + `memoryStatus`, so a user can continue a conversation across requests. `AgentConversationService` wraps the still-**Redis-free** `AgentOrchestrator` — it loads bounded prior-turn history, runs the loop with that history injected via a delimited, untrusted `{history}` prompt slot, then appends the bounded turn (user message + bounded tool summaries + assistant response) under a sliding 24h TTL. Memory lives in Redis as one JSON blob per conversation under `conv:{userId}:{conversationId}`; ownership is enforced two ways (userId-scoped key + asserted stored owner) so a guessed/foreign id → masked **404**. Failure is predictable (ADR-0019): an existing conversation with Redis down → `503` (fail-closed before any tool runs); a new conversation degrades to a stateless turn. `DELETE /api/v1/agent/conversations/{id}` lets a user delete their own memory. Verified with real-Redis Testcontainers, including a multi-turn test proving turn 2's prompt carries turn 1's context. **Hard** guardrail enforcement, human confirmation, and rate limiting are **M8**; durable audit is **M9.** Everything below marked _Planned_ describes the intended system, not shipped functionality. See [`docs/ROADMAP.md`](docs/ROADMAP.md) and [`docs/CHANGELOG.md`](docs/CHANGELOG.md).
...
- **Guardrails** — _implemented (M8):_ centralized `GuardrailEngine` (risk/argument policies), single-use fingerprint-bound confirmation for side-effecting/dangerous ops, per-user rate limiting, layered timeouts, loop detection; on top of M6's max tool calls / iteration / deadline bounds.
- **Security** — JWT auth, RBAC, per-resource ownership checks enforced _before_ any tool runs.
- **Memory** — Redis for short-term conversation memory (bounded, sliding-TTL, per-user) — _implemented (M7)_; PostgreSQL for durable data. Session/execution-state and caching rows are _planned_.
- **Observability & audit** — request-id + agent-execution correlation via MDC, low-cardinality Micrometer metrics scraped at `/actuator/prometheus`, DB-only readiness probe, and **enforced** audit retention (`AuditRetentionJob`, nightly, batched, cascading, best-effort) — _implemented (M10)_. A durable, owner-scoped **audit trail** of agent decisions, guardrail outcomes, confirmations, and tool executions (three typed PostgreSQL tables, read via `GET /api/v1/agent/executions`) — _implemented (M9)_. Full Prometheus/Grafana/OTel deployment is _planned (M12)_.
...
| Memory (M7) / guardrails (M8) / durable audit (M9) | ✅ Implemented |
| Observability & retention enforcement (M10) | ✅ Implemented |
| Observability dashboards | ⬜ Planned (M10) |
